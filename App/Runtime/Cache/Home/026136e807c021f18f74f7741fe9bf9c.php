...
<form id="pagerForm" data-toggle="ajaxsearch" action="/X-CRM3/index.php?s=/Home/Ruku" method="post">
        ...
        <div class="bjui-searchBar">
            <label>关键词：</label><input type="text" value="<?php echo ($_REQUEST['keys']); ?>" name="keys" class="form-control" size="12" />
             ...
             <button type="submit"  class="btn-default" data-icon="search">查询</button>
			  ...
			  <?php if(display(CONTROLLER_NAME.'/del')){ ?><span style="float:right;" ><a href="/X-CRM3/index.php?s=/Home/Ruku/del/navTabId/<?php echo CONTROLLER_NAME;?>" class="btn btn-red" data-toggle="doajax" data-confirm-msg="确定要清理吗？" data-icon="remove">清理</a></span> <?php } ?>
			  <?php if(display(CONTROLLER_NAME.'/outxls')){ ?><span style="float:right;margin-right:5px;"><a href="/X-CRM3/index.php?s=/Home/Ruku/outxls" class="btn btn-blue" data-toggle="doexport" data-confirm-msg="确定要导出吗？" data-icon="arrow-up">导出</a></span><?php } ?>
			  <?php if(display(CONTROLLER_NAME.'/add')){ ?><span style="float:right;margin-right:5px;"><a href="/X-CRM3/index.php?s=/Home/Ruku/add/navTabId/<?php echo CONTROLLER_NAME;?>" class="btn btn-green" data-toggle="dialog" data-width="900" data-height="500" data-id="dialog-mask" data-mask="true" data-icon="plus">新增</a></span><?php } ?>
		</div> 
</form>
...
<a href="/X-CRM3/index.php?s=/Home/Ruku/shenhe/id/<?php echo ($v['id']); ?>/navTabId/<?php echo CONTROLLER_NAME;?>"   class="btn btn-green btn-sm" data-toggle="dialog" data-width="900" data-height="500" data-id="dialog-mask" data-mask="true" ><?php echo ($v["shenhe"]); ?></a>
</td><?php } ?>
<td><?php echo (msubstr($v["uname"],0,20)); ?></td>
<td><?php echo (substr($v["addtime"],0,10)); ?></td>

		   <?php if(display(CONTROLLER_NAME.'/view')){ ?><td><a href="/X-CRM3/index.php?s=/Home/Ruku/view/id/<?php echo ($v['id']); ?>/navTabId/<?php echo CONTROLLER_NAME;?>"  data-toggle="dialog" data-width="900" data-height="500" data-id="dialog-mask" data-mask="true" >详细</a></td><?php } ?>
		   <?php if(display(CONTROLLER_NAME.'/edit')){ ?><td>
		   <?php if($v['shenhe'] == '审核通过'): ?><a href="/X-CRM3/index.php?s=/Home/Ruku/dayin/id/<?php echo ($v['id']); ?>/navTabId/<?php echo CONTROLLER_NAME;?>" class="btn btn-green btn-sm"  target="_blank" ><?php echo ($v["shenhe"]); ?>/打印</a>
		   <?php else: ?>
		   <a href="/X-CRM3/index.php?s=/Home/Ruku/edit/id/<?php echo ($v['id']); ?>/navTabId/<?php echo CONTROLLER_NAME;?>"   class="btn btn-green btn-sm" data-toggle="dialog" data-width="900" data-height="500" data-id="dialog-mask" data-mask="true" ><?php echo ($v["shenhe"]); ?>/编辑</a><?php endif; ?></td><?php } ?>
		   </td>
