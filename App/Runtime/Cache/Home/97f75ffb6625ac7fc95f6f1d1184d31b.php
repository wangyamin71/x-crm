...
<tr><td><label for='title_input' class='control-label x85'>客户名称:</label><input type='text' id='title' name='title'  size='20' data-toggle='lookup' data-url='/X-CRM3/index.php?s=/Home/basic/cust/navTabId/<?php echo CONTROLLER_NAME;?>'  value='<?php echo ($Rs["title"]); ?>'  ></td><td><label for='name_input' class='control-label x85'>联系人:</label><input type='text' id='name' name='name'  size='20'   value='<?php echo ($Rs["name"]); ?>'  ></td></tr>
<tr><td><label for='dianhua_input' class='control-label x85'>联系电话:</label><input type='text' id='dianhua' name='dianhua' data-rule='required;' size='20'   value='<?php echo ($Rs["dianhua"]); ?>'  ></td><td><label for='jingban_input' class='control-label x85'>业务员:</label><input type='text' id='jingban' name='jingban'  size='20' data-toggle='lookup' data-url='/X-CRM3/index.php?s=/Home/basic/jingban/navTabId/<?php echo CONTROLLER_NAME;?>'  value='<?php echo ($Rs["jingban"]); ?>'  ></td></tr>
<tr><td><label for='juname_input' class='control-label x85'>分享给:</label><input type='text' id='juname' name='juname'  size='20' data-toggle='lookup' data-url='/X-CRM3/index.php?s=/Home/basic/user/navTabId/<?php echo CONTROLLER_NAME;?>'  value='<?php echo ($Rs["juname"]); ?>'  ></td><td><label for='jztime_input' class='control-label x85'>截至日期:</label><input type='text' data-toggle='datepicker' id='jztime' name='jztime'   size='20'   value='<?php echo (substr($Rs["jztime"],0,10)); ?>'  ></td></tr>
...
<div style="margin:0 auto;width:88%;font-size:18px;padding:10px;text-align:center;">产品明细表
<table class="table table-bordered table-hover table-striped table-top"   data-toggle="tabledit">
                <thead>
                    <tr> 
                        <th title="序号" ><input type="text" size="5" name="mid[#index#].mid" value="" readonly  placeholder="ID"></th>
                        <th title="产品编号"><input type="text" name="xbianhao[#index#]" id="index[#index#].bianhao"  type="text"  value="" size="10" data-toggle="lookup" data-url='/X-CRM3/index.php?s=/Home/Basic/chanpin/navTabId/<?php echo CONTROLLER_NAME;?>' data-group="index[#index#]" ></th>
						...
						<th title="" data-addtool="true" width="50">
                            <a href="/X-CRM3/index.php?s=/Home/public/delok" class="btn btn-red row-del" data-confirm-msg="确定要删除该行信息吗？">删</a>
                        </th>
                    </tr>
                </thead>
                <tbody>
			    <?php $list = M('hetongs')->where(array("hid"=>$Rs['id']))->select(); ?>
...
<td>--</td>
			...
			<table class="table table-bordered table-hover table-striped table-top" >
                <tbody>
                  <Tr><td style="width:75%;text-align:right;">合计</td><Td><input name="jine" type="text" size="10" value="<?php echo ($Rs["jine"]); ?>"></td></tr>				  
                </tbody>
            </table>
 </div>
 </td></tr>
<tr><td colspan=2><input type='hidden' id='cid' name='cid'  size='0'   value='<?php echo ($Rs["cid"]); ?>'  ></td></tr>
